...
with open("error.log", "w", encoding="utf-8"):
    pass
...
async def algo_01(
    ...
    for asset in assets:
        ...
        point_abs *= random.uniform(1 - RANDOM_SCALING_RATIO, 1 + RANDOM_SCALING_RATIO)
        interval_abs = [
            interval_abs[0] * random.uniform(1 - RANDOM_SCALING_RATIO, 1 + RANDOM_SCALING_RATIO),
            interval_abs[1] * random.uniform(1 - RANDOM_SCALING_RATIO, 1 + RANDOM_SCALING_RATIO),
        ]
...
async def point_and_interval(request: Request, req: PointAndIntervalRequest) -> Dict[str, Any]:
    ...
    await asyncio.gather(candle_cache.wait_ready(), realtime_cache.wait_ready())
    ...
    if algo_name == "algo_default":
        ...
        resp = await algo_01(base_ts, normalized, req.extra, data_ready, scale_info, price_refs)
    elif algo_name == "algo_02":
        resp = await algo_02(base_ts, normalized, req.extra, data_ready, scale_info, price_refs)
    else:
        resp = await algo_default(base_ts, normalized, req.extra, price_refs)
...
async def error_log(request: Request) -> Dict[str, Any]:
    await _ensure_ip_allowed(request)
    try:
        with open("error.log", "r", encoding="utf-8") as fh:
            content = fh.read()
    except FileNotFoundError:
        content = ""
    return {"timestamp": iso_seconds(utc_now()), "log": content}
