...
from utils.bittensor import print_info, setup_bittensor_objects
from utils.cm_data import CMData
from utils.config import config
from utils.general import func_with_retry, loop_handler
from miners.routing_miner_latest import forward as routing_miner_forward  # Old single-asset prediction request
from miners.my_miner_latest import forward as my_miner_forward  # New multi-asset prediction request
...
class Miner:
    ...
    def __init__(self, config=None):
        ...
        # self.forward_module = importlib.import_module(f"miners.{config.forward_function}")
        self.config = config
        self.config.neuron.type = "Miner"
        setup_bittensor_objects(self)
        self.miner_id = f"{self.config.netuid}_{self.config.wallet.name}_{self.config.wallet.hotkey}_{random.randint(100000, 999999)}"
        bt.logging.debug(f"Miner ID: {self.miner_id}")
        self.cm = CMData()
    ...
    async def forward(self, synapse: Challenge) -> Challenge:
        bt.logging.info(f"New Challenge: ts={synapse.timestamp} | assets={synapse.assets}")
        if synapse.assets is None:
            synapse = await routing_miner_forward(synapse, self.cm, self.config.wallet.name, self.config.wallet.hotkey, self.miner_id)
            if synapse.prediction is not None:
                bt.logging.success(f"⚠️[OLD] Prediction: {synapse.prediction}  |  Interval: {synapse.interval}")
            else:
                bt.logging.error("No predictions for this request.")
        else:
            synapse = await my_miner_forward(synapse, self.cm, self.config.wallet.name, self.config.wallet.hotkey, self.miner_id)
            if synapse.predictions is not None:
                bt.logging.success(f"🚀[NEW] Predictions: {json.dumps(synapse.predictions)}  |  Intervals: {json.dumps(synapse.intervals)}")
            else:
                bt.logging.error("No predictions for this request.")
        return synapse
