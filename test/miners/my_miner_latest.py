...
def compose_payload(synapse: Challenge, coldkey_name: str, hotkey_name: str):
    ...
    return {
        ...
        "assets": synapse.assets or ["btc"],
        "extra": extra,
    }


async def forward_async(synapse: Challenge, cm: CMData, coldkey_name: str, hotkey_name: str, miner_id: str) -> Challenge:
    ...
    ok = False
    point_estimates, prediction_intervals = {}, {}
    try:
        if api_server_url == "":
        ...
        if r.status_code == 200 and response.get("ok", False) == True:
            ...
            for asset in assets:
                if asset in results:
                    ...
                    point_estimates[asset] = point_estimate
                    prediction_intervals[asset] = list(prediction_interval)
                    bt.logging.info(
                        f"{asset}: Prediction=${point_estimate:.2f} | Interval=[${prediction_interval[0]:.2f}, ${prediction_interval[1]:.2f}]"
                    )
                else:
                    bt.logging.warning(f"No prediction returned for {asset} from custom API.")
                    ok = False
                    break
            else:
                ok = True
    ...
    if ok == True:
        synapse.predictions = point_estimates
        synapse.intervals = prediction_intervals
        if synapse.predictions:
            bt.logging.success(f"Predictions complete for {list(point_estimates.keys())}")
        else:
...
async def forward(synapse: Challenge, cm: CMData, coldkey_name: str, hotkey_name: str, miner_id: str) -> Challenge:
    ...
    return await forward_async(synapse, cm, coldkey_name, hotkey_name, miner_id)
