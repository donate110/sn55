...
_DEFAULT_FLIP_RATE = 0.3
_DEFAULT_RANDOM_JITTER_RATIO = 0.03
...
class ConfigManager:
    ...
    def __init__(self, config_filename: str = "config.json") -> None:
        ...
        self._flip_rate: float = _DEFAULT_FLIP_RATE
        self._random_jitter_ratio: float = _DEFAULT_RANDOM_JITTER_RATIO
    ...
    def _extract_float(self, cfg: Dict[str, Any], key: str, default: float) -> float:
        try:
            value = float(cfg.get(key, default))
            return value
        except (TypeError, ValueError):
            log.warning("Invalid %s in config.json; using default %.3f", key, default)
            return default

    async def get_flip_rate(self) -> float:
        cfg = await self.get_config()
        return self._extract_float(cfg, "flip_rate", _DEFAULT_FLIP_RATE)

    async def get_random_jitter_ratio(self) -> float:
        cfg = await self.get_config()
        return self._extract_float(cfg, "random_jitter_ratio", _DEFAULT_RANDOM_JITTER_RATIO)
